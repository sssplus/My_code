...
// User-entered and AI-returned strings are rendered with innerHTML,
// so everything dynamic must be escaped to prevent stored XSS.
...
function tkAddCustom() {
  ...
  if (!name || name.length > 80 || isNaN(price) || price <= 0) return;
...
Be specific about functional overlap between the actual tools listed. One verdict per tool.`;
  ...
  try {
    ...
    tkState.audit = {
      ...
      estimatedMonthlySavings: Number(parsed.estimatedMonthlySavings) || 0,
...
function tkRender() {
  ...
  if (monthlyTotal > 0) {
    ...
    Object.entries(byCategory).forEach(([cat, v]) => {
      ...
      html += `<div class="tk-bar-seg" title="${esc(cat)}: ${tkFmt(v.total)}/mo" style="width:${pct}%; background:${CAT_COLORS[cat] || '#5A6B7E'}"></div>`;
    });
    html += `</div></div><div class="tk-legend">`;
    Object.entries(byCategory).forEach(([cat, v]) => {
      html += `<span class="tk-legend-item"><span class="tk-legend-dot" style="background:${CAT_COLORS[cat] || '#5A6B7E'}"></span> ${esc(cat)} · ${tkFmt(v.total)}</span>`;
    });
    html += `</div>`;
  } else {
  ...
  if (upcoming.length > 0) {
    ...
    upcoming.forEach(s => {
      html += `<div class="tk-renewal-row"><span class="tk-renewal-name">${esc(s.name)}</span><span class="tk-renewal-amt">${tkFmt(getDisplayPrice(s))} in ${s.days === 0 ? 'today' : s.days + 'd'}</span></div>`;
    });
    html += `</div>`;
  }
  ...
  PRESETS.forEach((p, idx) => {
    ...
    html += `<button class="tk-preset-btn ${added ? 'added' : ''}" ${added ? 'disabled' : ''} onclick="tkAddPreset(${idx})">
      ${added ? '✓ ' : '+ '}${esc(p.name)} <span class="tk-preset-price">${c}${displayPrice}</span>
    </button>`;
  });
  html += `</div></div>`;
  ...
  html += `
    <div class="tk-card tk-form-card">
      ...
      <div class="tk-form-grid">
        <input class="tk-input" id="tk-in-name" maxlength="80" placeholder="Tool name">
  ...
  if (subs.length > 0) {
    ...
    subs.forEach(s => {
      ...
      html += `
        <div class="tk-sub-row" style="border-left-color: ${CAT_COLORS[s.category] || '#5A6B7E'}">
          <div class="tk-sub-info">
            <div class="tk-sub-name">${esc(s.name)} ${dup ? `<span class="tk-overlap-tag">OVERLAP</span>` : ''}</div>
            <div class="tk-sub-meta">${esc(s.category)} · ${esc(s.cycle)} billing ${s.renewal ? '· renews ' + esc(s.renewal) : ''}</div>
          </div>
          <div class="tk-sub-price">${tkFmt(tkMonthly(s))}<span class="tk-sub-mo">/mo</span></div>
          <button class="tk-remove-btn" onclick="tkRemove('${esc(s.id)}')">✕</button>
        </div>
      `;
    });
    html += `</div></div>`;
  ...
  if (subs.length >= 2) {
    ...
    if (auditError) html += `<div class="tk-audit-error">${esc(auditError)}</div>`;

    if (audit) {
      html += `
        <div class="tk-audit-results">
          <div class="tk-audit-summary">${esc(audit.summary)}</div>
      ...
      (audit.verdicts || []).forEach(v => {
        const verdict = ['keep', 'cut', 'consolidate'].includes(v.verdict) ? v.verdict : 'consolidate';
        const cls = verdict === 'keep' ? 'tk-verdict-keep' : (verdict === 'cut' ? 'tk-verdict-cut' : 'tk-verdict-consolidate');
        html += `
          <div class="tk-verdict-row">
            <span class="tk-verdict-tag ${cls}">${verdict}</span>
            <span class="tk-verdict-text"><strong>${esc(v.name)}</strong> — ${esc(v.reason)}</span>
          </div>
        `;
      });
      html += `</div></div>`;
